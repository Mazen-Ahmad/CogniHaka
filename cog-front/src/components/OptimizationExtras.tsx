...
const OptimizationExtras: React.FC<OptimizationExtrasProps> = ({ capacityUtilization }) => {
  ...
  const handleGeminiQuery = async () => {
    ...
    try {
      ...
      setResponse('Error connecting to T-Cog');
    } finally {
      setLoading(false);
      setQuery('');
    }
  ...
  return (
    <div className="mt-8">
      <div className="flex gap-6">
        {/* Gemini AI Query Section - Now with fixed width */}
        <div className="w-1/2 bg-gray-50 p-4 rounded-lg border">
          <h3 className="text-lg font-semibold mb-3">T-Cog AI</h3>
          <div className="flex gap-2">
            ...
            >
              {loading ? 'Asking...' : 'Ask T-Cog'}
            </button>
          </div>
          
          {response && (
            <div className="mt-4 p-3 bg-white border rounded-lg">
              <p className="text-sm text-gray-600 font-medium">T-Cog's Response:</p>
              <p className="mt-1 text-gray-800">{response}</p>
            </div>
          )}
        </div>

        {/* Capacity Alert Section - Now with fixed width */}
        {capacityUtilization >= 100 && (
          <div className="w-1/2 bg-red-50 border border-red-200 p-4 rounded-lg h-fit">
